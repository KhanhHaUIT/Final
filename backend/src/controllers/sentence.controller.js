...
exports.postContributeSentence = async (req, res, next) => {
  try {
    ...
    if (isExist) {
      return res
        .status(409)
        .json({ message: 'Exist sentences, please try again' });
    }
    ...
    return res.status(503).json({ message: 'Error services, please try again' });
  } catch (error) {
...
exports.getSentenceList = async (req, res, next) => {
  try {
    ...
    return res.status(500).json({ message: 'Error services, please try again' });
  }
};
